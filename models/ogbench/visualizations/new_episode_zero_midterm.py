...
for frame_index in range(100):
    ...
    for camera_name in CAMERA_NAMES:
        ...
        rgb_frame_dir = os.path.join(
            f"new_outputs/rgb/frame_{frame_index:02d}/",
            f"{camera_name}.png"
        )
        os.makedirs(rgb_frame_dir, exist_ok=True)
...
np.savez_compressed(
   "outputs/multiview_data_only_filtered_pcds.npz",
    gt_points=np.array(all_points, dtype=object)
)
...
np.savez("outputs/multiview_data_filtered_background.npz", **save_dict)
